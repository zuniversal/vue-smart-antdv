...
export default defineComponent({
  setup(props, ctx) {
    return () => {
      ...
      const com = <button onClick={add}>{count.value}</button>;
      ...
      const getRes = () => {
        ...
        console.log(
          ' getRes  formRef.value.formRef.value ,   ： ',
      ...
      const slotsCom = {
        topSlot: () => (
          <div style={{ textAlign: 'center' }}>父组件传递 topSlot</div>
        ),
        bottomSlot: () => (
          <div style={{ textAlign: 'center' }}>父组件传递 bottomSlot</div>
        ),
      };

      const propsFn = params => {
        console.log(' propsFn   ,   ： ', params);
        varLabel.value = params.name;
      };
      const eventAttr = {
        propsFn,
      };
      ...
      return (
        <div className="">
          <a-button type="primary" onClick={getRes}>
            getRes
          </a-button>
